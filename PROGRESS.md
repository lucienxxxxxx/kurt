...
- **本轮桌面端更新(2026-06-29)**:`kurt-app` 侧边栏去掉新对话下的独立「项目」按钮,新增与「最近」同级的「项目」分区;按会话 workspace 全路径聚合,同一 workspace ≥3 个会话自动成项目,同名文件夹用父路径逐级消歧并做长文本省略;新增 **Skills** 页面,通过 bridge 扫描 Kurt/project/Codex/Agents/Claude/custom skill 目录,可查看正文并把技能调用带回 composer;新增 `@/components/ui/*` 本地 shadcn-style 包装(Button/Badge/ScrollArea)。`kurt-bridge` 新增 `/skills`/`/skills/:name`, `/sessions` 返回 workspace,每轮按会话 workspace 注入 skill catalog 并在所有模式提供 `skill` 工具。
...
- **门禁**:kurt-agent **169** · kurt-tui **98** · kurt-bridge **50** · kurt-app build+**Vitest 112**+cargo ✓ · 全 typecheck 干净(GUI 人工核对 `MANUAL_TESTS §6.3–§6.4` + Sidebar projects/skills page)
