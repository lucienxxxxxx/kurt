...
- **最后更新**:2026-06-29 · `main`(**chat/agent/plan 三模式定义收敛为单一真相源 `kurt-agent/src/agent/modes.ts`,两个前端共享、消除双写漂移**,见下方本轮条目)。历史:2026-06-29 · `main`(kurt-app Composer 模型菜单合并 effort:移除独立强度按钮,模型按钮显示当前强度,菜单内可直接切换;Composer.test 覆盖)。历史:2026-06-29 · `main`(v0.2.1 CLI 安装发布:新增 `scripts/install.sh`,支持 `curl -fsSL https://github.com/lucienxxxxxx/kurt/releases/latest/download/install.sh | sh`;新增 `bun run build:cli` 生成 `dist/kurt-<platform>-<arch>` 单文件 TUI CLI;为 Ink 编译显式加入 `react-devtools-core`;README 说明 npm 路线——用户安装无需账号,发布需要 npm 账号且裸名 `kurt` 已占用,建议 scoped package)。历史:2026-06-28 · `main`(v0.2.0 macOS 发布打包:README 开源化重写;版本同步到 0.2.0;`kurt-bridge` 通过 `bun build --compile` 生成 Tauri `externalBin` sidecar;发布版 app 优先运行 bundled `kurt-bridge`,开发版仍可用 Bun 源码入口;准备未签名 macOS `.app`/`.dmg` release)。历史:2026-06-27 · `main`(TUI `/mcp` + `/provider` 多渠道应用内配置,见下方本轮条目)。历史:2026-06-22 · `main`(**并发授权/询问改顺序队列(一个一个处理)**;**显示当前执行步骤(底部活动 + 工具 spinner)+ 回复结束重对账(防被吃)**;**上下文用量改用接口返回 token(双环 % 用真实 inputTokens,估算仅兜底)**;**通用授权 request_access(write/network/open,不再写死写目录)**;**修沙盒过窄 + 模型不会申请授权:可写根加系统临时目录、shell 写拒错追加 request_write_access 提示、prompt 明确沙盒规则**;**修 SSE「Load failed」:心跳保活 + bridge 崩溃兜底(LLM 重试已确认存在)**;**多模型提供商 阶段1(OpenAI/Claude/DeepSeek/自定义 + 启用开关 + 分组下拉;Claude 原生留阶段2)**;**markdown 改用 react-markdown + remark-gfm(支持 `>`/`*斜体*`/删除线/嵌套列表等)**;**workspace 按会话(composer 底部目录选择器,引擎工具/提示/文件树/终端全部 rooted 到会话目录)**;**会话全局统一列表(不再按 workspace 过滤)+ 标签最小宽度/禁横向滚动条**;**引擎并行工具调用(同一轮多个独立调用并发执行)**;**发送/完成音效 + 后台完成系统通知**;**system prompt 注入当前时间+系统信息(每轮)**;**对话条件式底部跟随 + 回到最新 + 流式淡入**;**工作区标签栏 Phase A+B+C + 分屏标签组 + 按会话独立 + 下拉层级**:分屏=两个编辑器组(每屏自带标签条);标签/分屏按会话各存一份;模式/模型/强度持久化;下拉菜单改 fixed 不被分屏裁切 + z-index 规范;单屏铺满宽度修复。**工作区标签栏 Phase A+B+C 全部完成**:标题下标签栏 + 自研左右分屏；会话/文件/预览/计划/**终端**标签，DetailPanel 统一进标签系统；bridge `/fs`·`/file`·`/raw` + `/info` 暴露 workspace + `plan` 帧；**自动触发**:计划→自动开计划标签、run 产出文档→自动开预览；**终端** = Rust portable-pty + xterm.js(懒加载)。前置:新 app 图标、统一步骤头、IN/OUT 截断、文件名点击预览、隐藏输入框滚动条、已思考 N秒)
- **本轮:三模式定义收敛(2026-06-29)**:chat/agent/plan 的**工具子集 + per-mode 提示 + normalizeMode** 原本在 kurt-tui(`agent.ts`)和 kurt-bridge(`runtime.ts`)**各写一份且已漂移**(TUI 只读集带 `skill`、bridge 带 `request_write_access`,提示文案也不同)。现统一到 **`kurt-agent/src/agent/modes.ts`**(单一真相源):`MODE_TOOLS`/`READ_ONLY_TOOLS`、`toolsForMode`(扁平列表)/`toolsForModeFromHub`(ToolHub)、`modeGuidance`、`normalizeMode`。两个前端改为薄包装消费(公开名 `systemPrompt`/`toolsForMode`/`TOOLS_BY_MODE`/`Mode` 全保留,调用点零改)。**统一后只读集** = `read_file/ls/grep/web_search/memory/ask_user/skill/request_access`(plan +`update_plan`;agent=all);`request_access` 特意留在只读模式(其 write 授权同时放宽共享可读根 = 读工作区外目录的逃生口),`request_write_access` 作向后兼容别名随 `request_access` 一起放行。**铁律 #3**:模式=编排数据,引擎从不见 Mode。新增 `modes.test.ts`(12 测,真实 Tool 列表断言 alias 行为)。门禁:kurt-agent 181(+12)· kurt-tui 98 · kurt-bridge 50 · 全 typecheck 干净(本轮在受限沙箱内开发,sandbox-exec/本地 HTTP 类用例需主机权限,数字以主机为准)。
- **本轮桌面端图标整理(2026-06-29)**:`kurt-app` 新增 `lucide-react` 依赖,`src/components/Icon.tsx` 从手写 SVG path 字典改为 Lucide-first 适配层,保留既有本地 icon name API 以避免调用点 churn;仅未知 icon 使用极小 fallback,DeepSeek 品牌 logo/上下文用量环等非通用图标继续自管 SVG。根 `AGENTS.md` 写入规则:前端通用图标优先 Lucide,业务组件通过 `Icon.tsx`,不得新增手写 SVG path 图标(品牌/可视化除外)。
- **本轮桌面端菜单整理(2026-06-29)**:`kurt-app` 将 Composer 底部的 effort/强度并入模型下拉菜单,移除独立强度按钮;模型按钮保留当前模型并显示当前强度短标签,下拉内可直接切换 Low/Medium/High/Max,thinking 开关继续留在同一模型菜单。新增 Composer RTL 测试覆盖菜单内切换 effort。
- **本轮桌面端更新(2026-06-29)**:`kurt-app` 侧边栏去掉新对话下的独立「项目」按钮,新增与「最近」同级的「项目」分区;按会话 workspace 全路径聚合,同一 workspace ≥3 个会话自动成项目,同名文件夹用父路径逐级消歧并做长文本省略,已归入项目的会话不再重复显示在「最近」;新增 **Skills** 页面,通过 bridge 扫描 Kurt/project/Codex/Agents/Claude/custom skill 目录,可查看正文并把技能调用带回 composer;新增 `@/components/ui/*` 本地 shadcn-style 包装(Button/Badge/ScrollArea)。`kurt-bridge` 新增 `/skills`/`/skills/:name`, `/sessions` 返回 workspace,每轮按会话 workspace 注入 skill catalog 并在所有模式提供 `skill` 工具。
...
- **门禁**:kurt-agent **181**(+12 modes.test) · kurt-tui **98** · kurt-bridge **50** · kurt-app build+**Vitest 116**+cargo ✓ · 全 typecheck 干净(GUI 人工核对 `MANUAL_TESTS §6.3–§6.4` + Sidebar projects/skills page)

---

## 一句话定位

main 处在「**单机 TUI Agent 主线完整可用 + macOS 桌面端可打包发布(Phase 6)**」的阶段:
七期里 1、2、3、**5 全部完成**,4 主线完成;**6 macOS 桌面主线功能完整并可产出未签名 release 包**(kurt-app:配置 key、选模型/effort、真实流式运行、真实会话列表/重载、敏感命令审批、bundled bridge sidecar);7 尚未开始。
...
| 6 | 多模态前端(WebUI/TUI/桌面/移动) | 🚧 TUI 成熟 · **macOS 桌面端可发布未签名包**(`kurt-app`,Tauri v2,见下方子阶段表)· 缺:Windows/WebUI/移动/签名公证 |
...
| 6.4d | 打包:`bun build --compile` bridge → Tauri `externalBin` sidecar 二进制;发布版 Rust shell 优先运行 bundled `kurt-bridge`,开发版保留 Bun 源码入口;`bun run build:mac` 产出 macOS bundle | ✅ 完成(未签名/未公证;Apple Developer ID 签名与 notarization 留发行配置) |
...
1. **Phase 4 余项**:**原生 Anthropic provider**(Messages API;TUI/bridge 目前只存 `format:"claude"` 仍走 OpenAI 兼容)/ 本地模型;`AuthProvider`(登录授权)。注:TUI 的 API key 已可在应用内配置(`/provider`,见下),不再只能走 env。
2. **Phase 6 余项**:Apple Developer ID 签名/公证、Windows/WebUI/移动前端。
...
- **MCP 工具只在 agent 模式可见**:chat/plan 取的是固定工具名单(现为共享的 `kurt-agent/src/agent/modes.ts` `MODE_TOOLS`),MCP 工具是动态名,
  目前不进 chat/plan —— 即便是只读 MCP 工具(readOnlyHint)。后续可让 `toolsForMode` 放只读 MCP 工具进 chat/plan(改 `modes.ts` 一处即两端生效)。
- **TUI 内 `/skills`、`/mcp`、`/provider` 发现/配置命令均已落地**(2026-06-27):`/skills` 列出已加载 skill;`/mcp` 列出已连 MCP 服务器(↵ 看其工具);`/provider` 在 TUI 内配置多渠道 API key(DeepSeek/OpenAI/Claude 预设 + Custom,持久化 `config.json` `providers`,首启引导,env 不再必需)。**Claude 渠道仍走 OpenAI 兼容传输**(原生 Anthropic 见上"未实现 §1")。
