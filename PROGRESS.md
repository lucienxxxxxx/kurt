...
- **最后更新**:2026-06-16 · `main` @ `f590266`(Phase 6.4c:APP 内配置 API key)
...
| 6.4c | **API key 设置 ✓**:Settings →「模型 / API」面板,粘贴 key 即存(`~/.kurt/desktop.json` 0600)并实时重建模型(`GET /info`+`POST /config`)。**待:** 模型/effort 菜单驱动真实 per-run config、真实文件预览、Keychain 加固 | 🚧 key ✓;余项待做 |
