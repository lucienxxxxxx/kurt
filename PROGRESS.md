...
- **最后更新**:2026-06-20 · `main`(**工作区标签栏 Phase A+B**:标题下标签栏 + 自研左右分屏；会话/文件/预览/**计划**标签，DetailPanel 统一进标签系统；bridge `/fs`·`/file`·`/raw` + `/info` 暴露 workspace + `plan` 帧；**自动触发**:计划→自动开计划标签、run 产出文档→自动开预览；终端仍占位(Phase C)。前置:新 app 图标、统一步骤头、IN/OUT 截断、文件名点击预览、隐藏输入框滚动条、已思考 N秒)
