...
| `src/components/` | `Icon`, `Markdown` (**`MdBlock` = react-markdown + remark-gfm**: blockquotes/`*italic*`/strike/tables/nested lists/task lists; class-mapped to keep the style; fenced code keeps lang label + copy button), `Sidebar`, `Composer` (+ menus, queue, run/stop), `Settings`, `thread/steps` (+ `renderStep`), **`Approval`** (sensitive-command modal → `approve()`) | 6.1/6.4b ✓ |
