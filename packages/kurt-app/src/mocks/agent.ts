/** Canned bilingual content (ported from prototype/data.js). Mock fixtures for
 *  6.1 static UI; replaced by the live kurt-bridge stream in 6.3. */

import type { LocalizedString, RawStep, Session } from "../types.ts";

const t = (zh: string, en: string): LocalizedString => ({ zh, en });
const U = (zh: string, en: string): RawStep => ({ type: "user", text: t(zh, en) });
const T_ = (zh: string, en: string): RawStep => ({ type: "text", text: t(zh, en) });
const TH = (sec: number, zh: string, en: string): RawStep => ({ type: "thinking", sec, text: t(zh, en) });
const TL = (name: string, tzh: string, ten: string, cmd: string, ozh: string, oen: string): RawStep => ({
  type: "tool", name, title: t(tzh, ten), cmd, out: t(ozh, oen),
});
const RD = (file: string, lines: string): RawStep => ({ type: "read", file, lines });
const SK = (name: string, tzh: string, ten: string, izh: string, ien: string, ozh: string, oen: string): RawStep => ({
  type: "skill", name, title: t(tzh, ten), input: t(izh, ien), output: t(ozh, oen),
});

// ---------- Session 1: organize downloads ----------
const downloads: Session = {
  id: "s1",
...
// ---------- live streaming run (when the user sends a new message) ----------
export const liveRun: RawStep[] = [
  TH(4,
  ...
  T_("我先把页面抓回来，看看返回是否正常。", "Let me fetch the page first and see if the response looks right."),
  SK("web_search",
    "搜索页面元信息", "Search page metadata",
    "site:example.com/blog/agent-design", "site:example.com/blog/agent-design",
  ...
  TL("Bash", "抓取页面 HTML", "Fetch the page HTML",
    ...
    "1842 /tmp/page.html", "1842 /tmp/page.html"),
...
// ---------- recents (sidebar) ----------
export const recents: { id: string; title: LocalizedString; icon: string }[] = [
...
const genericThread = (title: LocalizedString, qZh: string, qEn: string): Session => ({
  id: "", title, icon: "chat",
  steps: [
    U(qZh, qEn),
    TH(3, "理解需求，规划步骤，再逐步执行。", "Understand the goal, plan the steps, execute one by one."),
    T_("好的，我来处理「" + title.zh + "」。先确认环境和输入，再分步执行。",
       "Got it — I'll handle '" + title.en + "'. Let me confirm environment and inputs, then proceed step by step."),
    TL("Bash", "检查环境", "Check environment", "echo $SHELL && pwd", "/bin/zsh\n/Users/lew", "/bin/zsh\n/Users/lew"),
