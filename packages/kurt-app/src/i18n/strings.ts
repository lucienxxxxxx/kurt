...
export const T = {
  ...
  workspacePick: { zh: "工作目录", en: "Workspace" },
  ...
  apiProvidersTitle: { zh: "模型提供商", en: "Model providers" },
  apiProvidersDesc: { zh: "为内置厂商填 API 密钥并启用;或用「自定义」接入任意 OpenAI 兼容端点", en: "Add an API key and enable a built-in provider; or use Custom for any OpenAI-compatible endpoint" },
  apiEnable: { zh: "启用", en: "Enable" },
  apiModelsOptional: { zh: "模型(逗号分隔,留空用默认)", en: "Models (comma-separated; blank = defaults)" },
  apiClaudeSoon: { zh: "Claude 原生直连将在下一阶段提供;现在可填 key 并启用,运行会提示。", en: "Native Claude is coming next; you can add a key + enable now, runs will show a notice." },
