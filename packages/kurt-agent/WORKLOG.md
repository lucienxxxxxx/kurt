...
**根因与修复**:
- **失败原因被吞**(确定 bug):`runBee` 的 summary 取末轮叙述,真实 `failure`(maxTurns/API 错/abort)不进结果。→ `BeeResult.reason` 新增,蜂王把它放在结果**第一行**(`status: failed — <reason>`),输出裁剪永远裁不掉它。
- **beeMaxTurns 12 太小**(高概率根因):真实开发任务"读+写3文件+验证"轻松超 12 轮。→ 默认 32(用户问"无上限是否更好"——答否:上限是防失控烧钱的安全网,调大+可配置即可)。
- **run_code 的 CWD 是临时目录**(工蜂自己发现的坑):脚本相对路径全失效。→ `CodeToolOptions.cwd` 新增,kurt-tui 设为 workspace(脚本文件仍在临时目录,只改运行目录)。
- **usage 不向上汇总**:蜂群运行时 planner/工蜂/总结器的 usage 事件全被吃掉。→ 蜂王累计三方 usage 并发出**累积**totals 的 usage 事件,TUI 状态栏直接生效。

**验收**:kurt-agent **105**(+2)/ kurt-tui **58**;新测试:失败原因在首行、3 工蜂 usage 累计=300、`run_code` 以配置 CWD 运行。
