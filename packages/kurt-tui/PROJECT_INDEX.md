...
- **Run a command on the host (outside the sandbox)**: the agent calls **`host_shell`** — runs the command via `DirectSandbox` (no confinement: full filesystem + network) and asks the user to approve EVERY command. For things the sandboxed `shell` can't do (system installs, files outside the workspace, hardware/USB, GUI apps). Implemented as a second `ShellTool` instance (`name:"host_shell"`, `requireApproval:true`); agent-mode only.
- **Capability escalation (aligned with the desktop app)**: the agent calls **`request_access`** (one tool, kinds `write`/`network`/`open`) — same approval prompt. `write` opens a dir for write_file/shell/run_code for the rest of the session (`makeTools` shares one mutable writable-roots array + seeds `grants.dirs`); `network` flips `grants.network` so shell/run_code can reach the internet (resolved live via `allowNetwork`); `open` launches a file/URL in the default app (via an injected `openInDefaultApp`). A `request_write_access` alias keeps older callers working. "Always" persists the grant key in the allowlist. Grants are session-scoped (`AccessGrants`), shared across `/new`.
...
- Gate before merge: **`bun run typecheck && bun test`** (currently 98 tests, offline; SessionStore tests moved to kurt-agent in 6.2).
