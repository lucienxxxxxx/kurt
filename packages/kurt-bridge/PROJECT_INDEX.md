...
| `src/runtime.ts` | Engine composition: `createRuntime` (generic, injectable model/tools/**makeTitle** — testable) + `productionRuntime` (DeepSeek + core tools + sandbox + shared SessionStore + auto-title model call); `runTurn` (load/create session → stream events as frames → persist; a **new** session's title is left empty in the opening frame and auto-summarized via `rt.makeTitle` after the turn, falling back to the first user message) | `createRuntime`, `productionRuntime`, `runTurn`, `Runtime` |
